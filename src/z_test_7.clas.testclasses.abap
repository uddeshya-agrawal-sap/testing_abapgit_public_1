...
CLASS ltc_demo_mul_targets DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      act_results       TYPE STANDARD TABLE OF z_pv_1 WITH EMPTY KEY,
      sales_order_items TYPE STANDARD TABLE OF z_cds_v2_1 WITH EMPTY KEY,
      sales_order_items_2 TYPE STANDARD TABLE OF z_pv_1 WITH EMPTY KEY,
...
CLASS ltc_demo_mul_targets IMPLEMENTATION.
  ...
  METHOD class_setup.
    ...
    environment = cl_cds_test_environment=>create( i_for_entity = 'Z_PV_2' test_associations = 'X').
  ENDMETHOD.
  ...
  METHOD test_cdsfrwk_open_so_items_eur.
    ...
    ( carrid = 'AF' connid = 'CO_F' fldate = '20200101' )
     ( carrid = 'AA' connid = 'CO_A' fldate = '20200102' )
    ( carrid = 'AB' connid = 'CO_B' fldate = '20200102' )
    ( carrid = 'AC' connid = 'CO_C' fldate = '20200103' )
    ).
    ...
    environment->insert_test_data( i_data = sales_order_items_2 ).
    SELECT * FROM z_pv_1 INTO TABLE @DATA(act_result_3) ##DB_FEATURE_MODE[VIEWS_WITH_PARAMETERS] .
