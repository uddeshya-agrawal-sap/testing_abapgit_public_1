...
CLASS ltc_demo_mul_targets DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    DATA:
      ...
      act_result_3      TYPE  STANDARD TABLE OF z_cds_v1_1.
...
CLASS ltc_demo_mul_targets IMPLEMENTATION.
  ...
  METHOD class_setup.
   ...
   ( i_for_entity = 'Z_PV_2'  i_select_base_dependencies = abap_true )
    ( i_for_entity = 'Z_CDS_V2_2' i_select_base_dependencies = abap_true )
...
*
*    "Test
    SELECT * FROM z_pv_2 INTO TABLE @DATA(act_results) .
    SELECT * FROM z_cds_v2_2( pcid = 'AA' ) INTO TABLE @DATA(act_result_1).
    SELECT * FROM z_cds_v2_1 INTO TABLE @DATA(act_result_2) .
    SELECT * FROM z_cds_v1_1 INTO TABLE @DATA(act_result_3) .
  ENDMETHOD.
